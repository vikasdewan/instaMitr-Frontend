  ...
  return (
    <div className="bg-black text-white fixed top-0 left-0 px-4 border-r-2 border-r-gray-700 h-screen hidden md:flex">
              ...
              {item.text === "Notifications" &&
                ...
                showPopover && (
                  <Popover>
                    ...
                    <PopoverContent
                      onInteractOutside={handlePopoverClick}
                      className="bg-black w-full h-48 overflow-auto"
                    >
                      <div>
                        {likeNotiList.length == 0 ? (
      ...
      <Sheet open={openSearch} onOpenChange={setOpenSearch}>
        <SheetContent side="left" className="bg-black border-r border-white w-[80%] sm:w-[60%] md:w-[400px] text-white">
          <SheetHeader>
            <SheetTitle className="text-2xl font-bold text-white">Search</SheetTitle>
          </SheetHeader>
