...
const Reels = () => {
  ...
  useEffect(() => {
    const fetchReels = async () => {
      try {
        const res = await axios.get(`https://instamitr-backend.onrender.com0/api/v1/post/all`, {
          headers: {
  ...
  const likeOrDislikeHandler = async () => {
    try {
      ...
      const res = await axios.get(
        `https://instamitr-backend.onrender.com0/api/v1/post/${currentReel._id}/${action}`,
        { withCredentials: true }
      );
